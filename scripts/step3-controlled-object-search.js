...
const SEARCHABLE_FIELDS = ["artist", "title"];
...
function renderObjectResults(items) {
  ...
  items.forEach(item => {
    ...
    li.textContent = `Artist: ${item.artist} | Song Title: ${item.title} | Album: ${item.album}`;
...
function handleControlledObjectSearch() {
  ...
  if (!query) {
    ...
    searchMessage.textContent = "Type to search artists";
    return;
  }
